...
evap <- evap[, .(grid_id, date, value, lon, lat)]

#line missing
saveRDS(evap, paste0(PATH_OUTPUT_DATA, region, '_evap_grid.rds'))

# save the grid lookup (lon/lat -> grid_id)
saveRDS(evap_grid, paste0(PATH_OUTPUT_DATA, region, '_evap_grid2.rds'))

## Pentads
pentads <- copy(evap)
pentads[, pentad := ceiling((yday(date) - leap_year(year(date)) * (yday(date) > 59)) / 5 )]
pentads[, std_value := (value - mean(value)) / sd(value), .(pentad, grid_id)]
pentads[, pentad_std_q95 := quantile(std_value, EXTREMES_THRES, na.rm = TRUE), by = grid_id] # added , na.rm = TRUE
pentads[, pentad_std_q80 := quantile(std_value, LOW_THRES, na.rm = TRUE), by = grid_id] # added , na.rm = TRUE
# pentads[, pentad_median_qr := rq(std_value ~ date,  #quantile regression: non-stationarity
#                                  tau = 0.5)$fitted, .(pentad, grid_id)] # for GLEAM 4.1 create ptoblem -- Error in rq.fit.br(x, y, tau = tau, ...) : Singular design matrix
# solved here===
library(purrr)
# pentads[, pentad_median_qr := tryCatch(rq(std_value ~ as.numeric(date), tau = 0.5)$fitted,
#   error = function(e) rep(NA, .N)), .(pentad, grid_id)]
pentads[, pentad_median_qr := tryCatch(rq(std_value ~ as.numeric(date), tau = 0.5)$fitted,
    error = function(e) rep(NA, .N)), .(pentad, grid_id)]

# pentads[, pentad_std_q95_qr := rq(std_value ~ date, 
#                                   tau = EXTREMES_THRES)$fitted, .(pentad, grid_id)] #Error in rq.fit.br(x, y, tau = tau, ...) : Singular design matrix

pentads[, pentad_std_q95_qr := tryCatch(rq(std_value ~ as.numeric(date), tau = EXTREMES_THRES)$fitted,
  error = function(e) rep(NA, .N)), .(pentad, grid_id)] #Added NA_real_ for NA
...
exeves[, period := factor('up_to_2001', levels = c('up_to_2001', 'after_2001'), ordered = TRUE)]
exeves[date > END_PERIOD_1, period := factor('after_2001', levels = c('up_to_2001', 'after_2001'), ordered = TRUE)]
...
exeves_qr[, above_low_thres_id := rleid(value_above_low_thres), .(grid_id)]
exeves_qr[, extreme_qr_id := rleid(extreme), .(grid_id)]
...
exeves_80_95[, above_low_thres_id := rleid(value_above_low_thres), .(grid_id)]
exeves_80_95[, extreme_id := rleid(extreme), .(grid_id)]
...
prec[, q95 := quantile(value, probs = 0.9, na.rm = TRUE), .(grid_id)]
...
lwrad[, std_value := (value - mean(value)) / sd(value), .(pentad, grid_id)][, pentad := NULL]
swrad[, std_value := (value - mean(value)) / sd(value), .(pentad, grid_id)][, pentad := NULL]
...
temp[, std_value := (value - mean(value)) / sd(value), .(pentad, grid_id)][, pentad := NULL]
