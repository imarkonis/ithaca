...
# # Function to log-scale ratio
# log_scale_ratio <- function(dt, ratio_col = "ratio") {
#   if (!ratio_col %in% names(dt)) stop("Column not found")
#   min_val <- min(dt[[ratio_col]], na.rm = TRUE)
#   shift_val <- ifelse(min_val <= 0, abs(min_val) + 0.01, 0)
#   dt[, paste0(ratio_col, "_pos") := get(ratio_col) + shift_val]
#   dt[, paste0(ratio_col, "_log") := log(get(paste0(ratio_col, "_pos")))]
#   # Scale (normalize to 0–1 range)
#   log_col <- paste0(ratio_col, "_log")
#   log_min <- min(dt[[log_col]], na.rm = TRUE)
#   log_max <- max(dt[[log_col]], na.rm = TRUE)
#   dt[, paste0(ratio_col, "_log_scaled") := 
#        (get(log_col) - log_min) / (log_max - log_min)]
#   return(dt)
# }
...
## Severity: ExEvEs
# event_evap_severity_period <- exeves[!is.na(event_id), .(value = sum(value)), .(lon, lat, period, KG_class_2)] 
# event_evap_severity_period[, diff_value := diff(value), by = .(lon, lat, KG_class_2)]
# event_evap_severity_period[, ratio := 1 + diff_value/value, by = .(lon, lat, period, KG_class_2)]
...
# setorder(event_evap_severity_period, lon, lat, KG_class_2, period)
# event_evap_severity_period[, diff_value := diff(value), by = .(lon, lat, KG_class_2)]
...
## Intensity
# event_evap_intensity_period <- exeves[!is.na(event_id), .(value = round(mean(value), 2)), .(lon, lat, period, KG_class_2)]
