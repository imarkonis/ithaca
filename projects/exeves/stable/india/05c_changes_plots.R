...
# borders <- read_sf('../../shared/data/geodata/maps/admin/INDIA/CZE_adm0.shp')
borders <- read_sf("~/shared/data/geodata/maps/admin/india/india_st.shp")
st_crs(borders) <- 4326


# | Column           | Meaning                                                                                                            |
#   | ---------------- | ------------------------------------------------------------------------------------------------------------------ |
#   | `event_qr_id`    | IDs for **evaporation events** detected using a quantile rule (e.g., events above the 80th percentile).            |
#   | `extreme_qr_id`  | IDs for **extreme evaporation events**, typically a subset of the most intense ones (e.g., above 95th percentile). |
#   | `event_80_95_id` | Marks events that fall between the **80th and 95th percentile** — these are *moderately extreme* events.           |
#   | `event_80_id`    | Marks events above the **80th percentile**, possibly a broader definition of “extreme.”                            |
#   
...
to_plot <- evap[, .(`All days` = mean(value)), by = .(year = year(date))]
to_plot <- merge(to_plot, evap[!is.na(extreme_qr_id), .(ExEvEs = mean(value)), by = .(year = year(date))],
                 by = 'year', all = TRUE)
to_plot <- merge(to_plot, evap[is.na(extreme_qr_id), .(`Non-ExEvEs` = mean(value)), by = .(year = year(date))],
                 by = 'year', all = TRUE)
to_plot <- melt(to_plot, id.vars = 'year', variable.name = "Conditions")
...
to_plot <- evap[, .(`All days` = sum(value) / GRID_CELL_N), year(date)]
to_plot <- merge(to_plot, evap[!is.na(extreme_qr_id), .(ExEvEs = sum(value) / GRID_CELL_N), year(date)], by = 'year')
to_plot <- merge(to_plot, evap[is.na(extreme_qr_id), .(`Non-ExEvEs` = sum(value) / GRID_CELL_N), year(date)], by = 'year')
to_plot <- melt(to_plot, id.vars = 'year', variable.name = "Conditions")
...
monthly_plot_sum <- ggplot() +
  ...
  geom_col(data = to_plot[stat == 'median' & Conditions == 'ExEvEs'], aes(x = month, y = value, fill = value), position = "dodge2", show.legend = TRUE, alpha = .8) +
  geom_point(data = to_plot[stat == 'median' & Conditions == 'All days'], aes(x = month, y = value), size = 1.5, color =  '#536878') +
  geom_line(data = to_plot[Conditions == 'All days'], aes(x = month, y = value), color =  '#536878') +
  geom_segment(data = to_plot[stat == 'median' & Conditions == 'ExEvEs'], aes(x = month, y = 0, xend = month,
                 yend = 8), color =  '#536878', linetype = 'dotted') + 
  coord_polar() +
  annotate(x = 11.5, y = 1.3, label = "1", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  annotate(x = 11.5, y = 2.3, label = "2", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  annotate(x = 11.5, y = 3.3, label = "3", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  annotate(x = 11.5, y = 4.3, label = "4", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  annotate(x = 11.5, y = 5.3, label = "5", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  annotate(x = 11.5, y = 6.3, label = "6", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  annotate(x = 11.5, y = 7.3, label = "7", geom = "text", color = 'grey50', family = "Bell MT", size = 2.6) +
  scale_y_continuous(limits = c(-1.5, 8.5)) + 
  # scale_fill_gradientn("Ratio", colours = c('grey97', colset_subdued_prof[2])) +
  scale_fill_gradientn("Ratio", colours = c('grey97', colset_subdued_prof[2]), limits = c(0, 8), breaks = seq(0, 8, 1))+
  # scale_fill_gradientn("Ratio", colours = c("#DED9D9", "#9DB0A9", "#C94730", "#7cb47c", "#fcc47c", "#FFA500"), limits = c(0, 8), breaks = seq(0, 8, 1))+
  guides(fill = guide_colorsteps(barwidth = 9, barheight = .5, title.position = "top", title.hjust = .5, size = 6))+
...
spatial_plot <- ggplot(spatial_changes[period == "up_to_2001" & variable == "Evaporation (ExEvEs)"]) +
  ...
  scale_x_continuous(breaks = seq(INDIA_LON_MIN, INDIA_LON_MAX, 2)) +
  scale_y_continuous(breaks = seq(INDIA_LAT_MIN - 0.5, INDIA_LAT_MAX, 1)) +
  ...
  guides(fill = guide_colorsteps(title.position = "top", title.hjust = 0.5, barwidth = 9, barheight = .5),
  color = guide_legend(title.position = "top", label.position = "bottom", order = 1))
...
ggarrange(gg_1, gg_2, nrow = 2) +
  bgcolor("white")

# ggsave(paste0(PATH_OUTPUT_FIGURES, final_plot, "exeve_changes_india.pdf"), width = 12, height = 10, bg="white", device = cairo_pdf)
ggsave(filename = paste0(PATH_OUTPUT_FIGURES, "exeve_changes_india_gleam4.1.pdf"), width = 12, height = 10, bg = "white", device = cairo_pdf)
