...
exeves_drivers <- merge(exeves, rad, by = c('grid_id', 'date'), all.x = TRUE) # all exeves col. for climate
...
exeves_drivers <- exeves_drivers[, .(grid_id, date, conditions, event_day, event_duration,
                   evap = evap, swrad, std_swrad, lwrad, std_lwrad, temp, sensible, prec, KG_class_2)]
