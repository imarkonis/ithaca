...
borders <- read_sf("~/shared/data/geodata/maps/admin/india/india_outline.shp")
...
# All days
exeves_prec_sums <- unique(exeves_prec[, .(evap = sum(evap) / (SUB_PERIOD_YEARS * DAYS_IN_YEAR), 
...
exeves_prec_sums <- merge(evap_grid, exeves_prec_sums, by = "grid_id", allow.cartesian = TRUE)

to_plot <- copy(exeves_prec_sums)
## the below line causing problem in unexpeted shutdown
...
to_plot[, Conditions := factor("Unknown", levels = c('Wetter - Accelerated', 'Wetter - Deccelerated', 'Drier - Accelerated', 'Drier - Deccelerated'))]
...
# Exeve days
exeves_prec_sums <- unique(exeves_prec[!is.na(event_80_95_id), .(evap = sum(evap) / (SUB_PERIOD_YEARS * DAYS_IN_YEAR), 
                                                            prec = sum(prec) / (SUB_PERIOD_YEARS * DAYS_IN_YEAR), 
                                                            diff_pe = (sum(prec) - sum(evap)) / (SUB_PERIOD_YEARS * DAYS_IN_YEAR)), 
                                        by = .(grid_id, period)])
exeves_prec_sums <- exeves_prec_sums[, .(prec, evap, diff_pe, diff_prec = diff(prec), diff_evap = diff(evap), period), .(grid_id)]
...
# to_plot[, Conditions := factor("Uknown")]
# levels(to_plot$Conditions) <- c('Wetter - Accelerated', 'Wetter - Deccelerated', 'Drier - Accelerated', 'Drier - Deccelerated')
to_plot[, Conditions := factor("Unknown", levels = c('Wetter - Accelerated', 'Wetter - Deccelerated', 'Drier - Accelerated', 'Drier - Deccelerated', "Unknown"))]
...
ggsave(paste0(PATH_OUTPUT_FIGURES, "implications_ind.png"), width = 8, height = 12)
