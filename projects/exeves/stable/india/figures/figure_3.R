...
event_evap_intensity_climate[, diff_value := before - after]

event_evap_intensity_climate[, ratio := after / before]

summary_by_climate <- event_evap_intensity_climate[
  !is.na(ratio), .(mean_ratio = mean(ratio), mean_diff = mean(diff_value)), by = KG_class_2]
...
###
scale_factor <- max(summary_by_climate_clean$mean_diff) / max(summary_by_climate_clean$mean_ratio)
...
ggplot(summary_by_climate_clean, aes(x = KG_class_2)) +
  ...
  scale_y_continuous(name = "Mean diff. (abs.)",
...
summary_by_climate <- event_frequency_period[, .(mean_ratio = mean(ratio, na.rm = TRUE),
      mean_diff = mean(diff_value, na.rm = TRUE)), by = KG_class_2]
