...
prec_stats <- prec_evap[, .(prec_mean = mean(prec), 
                            prec_sd = sd(prec)), .(lon, lat, dataset)]
...
## Merger 
dataset_ranks <- merge(
  prec_comparison[, .(lon, lat, dataset, 
                      prec_mean_bias = bias_mean, prec_sd_bias = bias_sd, 
                      prec_mean_rank = rank_mean, prec_sd_rank = rank_sd)], 
  evap_comparison[, .(lon, lat, dataset, 
                      evap_mean_bias = bias_mean, evap_sd_bias = bias_sd, 
                      evap_mean_rank = rank_mean, evap_sd_rank = rank_sd)],
      by = c("lon", "lat", "dataset"))

dataset_stats$dataset <- factor(dataset_stats$dataset)
dataset_ranks$dataset <- factor(dataset_ranks$dataset)
