...
water_avail <- grid_flux[, .(water_avail = mean(avail)), .(period, member)]
water_avail_wide <- dcast(water_avail, member ~ period, value.var = 'water_avail')
water_avail_wide[, avail_change := aft_2001 - pre_2001]

water_flux <- grid_flux[, .(water_flux = mean(flux)), .(period, member)]
water_flux_wide <- dcast(water_flux, member ~ period, value.var = 'water_flux')
water_flux_wide[, flux_change := aft_2001 - pre_2001]
