...
prec_data <- prec_data[biome_short_class != "Water" & biome_short_class != "Polar"]

prec_data[, rankings := frankv(prec_t, order = -1), biome_short_class]
...
data_levels <- rev(c("MSWEP v2.8", "GPM IMERG v7", "GPCP CDR v3.2", "CMAP",
                     "FLDAS", "PERSIANN CDR", "MERRA-2 Land", "EM-Earth",
                     "MERRA-2", "ERA5-Land", "GPCC FD v2022", "TerraClimate",
                     "CPC-Global", "JRA-55", "PREC/L", "ERA5", "CRU TS v4.08",
                     "GSMaP v8", "CMORPH CDR", "NCEP/DOE R2", "NCEP/NCAR R1"))

prec_data[, dataset := factor(dataset, levels = data_levels)]

ggplot(prec_data, aes(y = dataset)) +
  geom_bar(aes(fill = rank_group, x = 100*after_stat(count)/10),
           color = "gray23",
  ...
  labs(x = "Percentage of Biomes", y = "Dataset", fill = "Rank") +
  theme_bw() +
...
ggsave(paste0(PATH_SAVE_UNCERTAINTY_PREC_FIGURES, "biome_rank_table.png"),
