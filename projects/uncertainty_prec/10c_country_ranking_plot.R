...
prec_data <- prec_data[country != ""]

prec_data[, rankings := frankv(prec_t, order = -1), country]
...
data_levels <- rev(c("GPM-IMERG v7", "PREC/L", "MSWEP v2.8", "MERRA-2 Land",
                     "CRU TS v4.07", "PERSIANN-CDR", "EM-Earth", "FLDAS",
                     "ERA5-Land", "CMAP", "TerraClimate", "CPC-Global", "ERA5",
                     "GPCC v2020", "GSMaP v8", "JRA-55", "GPCP v3.2",
                     "NCEP/NCAR R1", "NCEP/DOE R2", "CMORPH CDR"))

prec_data[, dataset := factor(dataset, levels = data_levels)]

p00 <- ggplot(prec_data, aes(y = dataset)) +
  geom_bar(aes(fill = rank_group, x = 100*after_stat(count)/188),
           color = "gray23",
  ...
  labs(x = "Percentage of Countries", y = "Dataset", fill = "Rank") +
  theme_bw() +
...
ggsave(plot = p00,
       paste0(PATH_SAVE_UNCERTAINTY_PREC_FIGURES, "country_rank_table.png"),
